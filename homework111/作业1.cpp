        ...
        for(i = 0; i < n; i++) {
            ...
            for(j = i + 1; j < n; j++) { // 从 i+1 开始遍历，避免重复计算
                if(arr[i] == arr[j]) { // 如果找到相同的元素
                    count++; // 计算出现次数
                    if(count == 1) { // 第一次出现该元素，标记为已找到一次
                        flag = j;
                    }
                }
            }
            if(count == 1) { // 只出现一次，且之前已标记找到一次
                result[rsize++] = arr[flag]; 
            }
        }
   }
...
int main() {
    ...
    CountLetterWeight clw;
    cout << clw.countWeight(s) << endl;
